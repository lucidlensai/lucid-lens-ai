const cfg=window.LLAI_CONFIG||{};
  ...
  function buildCard(rec,i){
    ...
    var videoRaw=f['Video URL']||'',vid=driveId(videoRaw),videoView=driveViewUrl(videoRaw);
    ...
    var vd=vid?'<div class="detail-section-label" style="margin-top:1.5rem;">Dream Vision</div><div class="video-section"><div class="video-player-wrap"><iframe src="https://drive.google.com/file/d/'+vid+'/preview" width="100%" height="315" allow="autoplay" allowfullscreen style="border:none;border-radius:12px;display:block;"></iframe></div><div class="video-actions"><a class="audio-dl-btn" href="'+videoView+'" target="_blank">Download video</a></div></div>':'';
    return '<div class="dream-item" id="dream-'+id+'" data-mood="'+mood+'" data-interp="'+interp.substring(0,200).replace(/"/g,'&quot;')+'"><div class="dream-item-header" onclick="toggleDream(\''+id+'\')"><div class="dream-item-left"><div class="dream-item-date">'+date+'</div><div class="dream-item-text">'+txt+'</div>'+(mood?'<div class="dream-item-mood">'+mood+'</div>':'')+'</div><div class="dream-item-right"><span class="dream-status complete">Complete</span><span class="dream-chevron">v</span></div></div><div class="dream-detail">'+(interp?'<div class="detail-section-label">Interpretation</div><div class="detail-interpretation">'+interp+'</div>':'')+(img1||img2?'<div class="detail-section-label">Dream Visions</div><div class="detail-images"><div class="detail-img-wrap">'+i1+dl1+'</div><div class="detail-img-wrap">'+i2+dl2+'</div></div>':'')+au+vd+'</div></div>';
  }
